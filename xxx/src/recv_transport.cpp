...
void recv_transport_linux::open() {
	...
	if (_socket == -1) {
		...
		return;//throw std::exception("void recv_transport_linux::open(),Error creating raw socket");
	}
	...
	if ((ioctl(_socket, SIOCGIFINDEX, &ifr)) == -1) {
		...
		return;//throw std::exception("void recv_transport_linux::open(),Error getting Interface index");
	}
	...
	if ((bind(_socket, (struct sockaddr *)&sll, sizeof(sll))) == -1)
	{
		...
		return;//throw std::exception("void recv_transport_linux::open(),Error binding raw socket to interface");
	}
}
...
void recv_transport_linux::recv(std::vector<char> &data) {
	...
	if (result > sizeof(ethhdr)) {
		data.resize(result - sizeof(ethhdr));
		std::copy(packet + sizeof(ethhdr), packet + result, data.begin());
	}
}
